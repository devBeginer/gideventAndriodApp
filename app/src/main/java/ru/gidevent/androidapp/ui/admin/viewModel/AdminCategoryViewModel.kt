...
class AdminCategoryViewModel @Inject constructor(
    ...
    fun initCategory() {
        viewModelScope.launch {
            ...
            val response = advertRepository.getAllCategory()

            when (response) {
                is ApiResult.Success<List<Category>> -> {
    ...
    fun deleteCategory(id: Long) {
        viewModelScope.launch(Dispatchers.IO){
            dataResultMutableLiveData.postValue(UIState.Loading)
            val response = advertRepository.deleteCategory(id)
            when (response) {
    ...
    suspend fun editCategory(category: Category): UIState {
        val response = advertRepository.putCategory(category)
