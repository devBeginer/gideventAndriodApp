...
@AndroidEntryPoint
class CityBottomSheetDialog(private val viewModel: CreateAdvertViewModel): BottomSheetDialogFragment() {
    ...
    private fun initData() {
        adapter = CityPickRecyclerViewAdapter(listOf()){suggestionCity->
            viewModel.postCity(City(suggestionCity.id, suggestionCity.name))
            dismiss()
        }
